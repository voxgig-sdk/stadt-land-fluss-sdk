# StadtLandFluss SDK test runner
...
class StadtLandFlussTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            StadtLandFlussTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = StadtLandFlussTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = StadtLandFlussTestRunner.getenv("STADT_LAND_FLUSS_TEST_LIVE")
        override = StadtLandFlussTestRunner.getenv("STADT_LAND_FLUSS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = StadtLandFlussTestRunner.getenv(key)
        ...
        explain = StadtLandFlussTestRunner.getenv("STADT_LAND_FLUSS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["STADT_LAND_FLUSS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if StadtLandFlussTestRunner._test_control is not None:
            return StadtLandFlussTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                StadtLandFlussTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            StadtLandFlussTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = StadtLandFlussTestRunner.load_test_control()
...
def load_env_local():
    StadtLandFlussTestRunner.load_env_local()


def env_override(m):
    return StadtLandFlussTestRunner.env_override(m)


def entity_data(v):
    return StadtLandFlussTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return StadtLandFlussTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return StadtLandFlussTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return StadtLandFlussTestRunner.load_test_control()


def live_client_options():
    return StadtLandFlussTestRunner.live_client_options()


def live_delay_ms():
    return StadtLandFlussTestRunner.live_delay_ms()
