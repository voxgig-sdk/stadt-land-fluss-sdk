...
from stadtlandfluss_sdk.core.helpers import get_ctx_prop, to_map
...
class StadtLandFlussContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = StadtLandFlussControl()
        ...
        if isinstance(sp, StadtLandFlussSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, StadtLandFlussResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, StadtLandFlussResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return StadtLandFlussOperation({})
    ...
    def make_error(self, code, msg):
        return StadtLandFlussError(code, msg, self)
