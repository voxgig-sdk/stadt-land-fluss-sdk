...
//     (\`new StadtLandFlussSDK(...)\` / \`StadtLandFlussSDK.test(...)\` become
//     \`StadtLandFlussSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'StadtLandFlussSDK'
...
// rewrite real client construction (\`new StadtLandFlussSDK(...)\` and any
// \`StadtLandFlussSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+StadtLandFlussSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/StadtLandFlussSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
