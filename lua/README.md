# StadtLandFluss Lua SDK



The Lua SDK for the StadtLandFluss API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Data()` — each with the same small set of operations (`list`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/stadt-land-fluss-sdk/releases)),
...
Check `err` first (it is non-`nil` on failure), then use `value`:
    ...
    local data, err = client:Data():list()
    if err then error(err) end
    -- data is the record list
...
```
lua/
├── stadt-land-fluss_sdk.lua    -- Main SDK module
...
The main module (`stadt-land-fluss_sdk`) exports the SDK constructor
