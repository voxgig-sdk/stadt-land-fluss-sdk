...
// StadtLandFluss SDK retry feature
...
class StadtLandFlussRetryFeature extends StadtLandFlussBaseFeature
{
    ...
    public function init(StadtLandFlussContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (StadtLandFlussContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(StadtLandFlussContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(StadtLandFlussContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
