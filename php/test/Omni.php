...
//    only - a typed StadtLandFlussContext there would make every ctx
...
final class StadtLandFlussOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new StadtLandFlussContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new StadtLandFlussSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new StadtLandFlussResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new StadtLandFlussError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new StadtLandFlussResponse($clean['response']);
    ...
    public static function err_from_map($m): ?StadtLandFlussError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new StadtLandFlussError(is_string($code) ? $code : '', $msg);
    }
