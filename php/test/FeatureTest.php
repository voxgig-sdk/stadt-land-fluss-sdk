...
// StadtLandFluss SDK feature test
//
...
class FtCtrl extends StadtLandFlussControl
{
...
class FtClient
{
    ...
    private StadtLandFlussUtility $utility;

    public function __construct(StadtLandFlussUtility $utility, array $options)
    {
    ...
    public function get_utility(): StadtLandFlussUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (StadtLandFlussContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public StadtLandFlussContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (StadtLandFlussContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = StadtLandFlussConfig::make_feature($name);
    ...
    private static function buildUrl(StadtLandFlussSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof StadtLandFlussError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new StadtLandFlussResponse($fetched);
        }
    ...
    private function fail(StadtLandFlussContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (StadtLandFlussContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
