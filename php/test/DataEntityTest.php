...
class DataEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = StadtLandFlussSDK::test($seed, null);
        $seen = iterator_to_array($base->Data(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = StadtLandFlussConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = StadtLandFlussSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set STADT_LAND_FLUSS_TEST_DATA_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function data_basic_setup($extra)
{
    ...
    $client = StadtLandFlussSDK::test($options, $extra);
    ...
    $entid_env_raw = getenv("STADT_LAND_FLUSS_TEST_DATA_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["STADT_LAND_FLUSS_TEST_DATA_ENTID"]);
    ...
    if ($env["STADT_LAND_FLUSS_TEST_LIVE"] === "TRUE") {
        ...
        $client = new StadtLandFlussSDK(Helpers::to_map($merged_opts) ?? []);
    }
