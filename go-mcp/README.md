# stadt-land-fluss-mcp

[MCP](https://modelcontextprotocol.io) server exposing the StadtLandFluss SDK as
two agent tools — `stadt-land-fluss_list` and `stadt-land-fluss_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/stadt-land-fluss-mcp)
make build

# 2. Provide credentials via the environment
export STADT_LAND_FLUSS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user stadt-land-fluss \
  -- /absolute/path/to/stadt-land-fluss-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./stadt-land-fluss-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// stadt-land-fluss_list: first page of records
{ "entity": "data" }
{ "entity": "data", "query": { } }

// stadt-land-fluss_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/stadt-land-fluss-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user stadt-land-fluss \
     -- "$PWD"/dist/*/stadt-land-fluss-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `stadt-land-fluss_list` and `stadt-land-fluss_load` tools now appear
   in new sessions. Ask the agent to *"list data using stadt-land-fluss"*
   and it calls `stadt-land-fluss_list` with `{"entity":"data"}`.
...
```sh
export STADT_LAND_FLUSS_APIKEY=sk_live_xxx            # API key
export STADT_LAND_FLUSS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `stadt-land-fluss_list` tool
...
### Call the `stadt-land-fluss_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `stadt-land-fluss_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `stadt-land-fluss_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `STADT_LAND_FLUSS_APIKEY` | API key sent with every request. |
| `STADT_LAND_FLUSS_BASE` | Optional override of the API base URL. |
...
```sh
./stadt-land-fluss-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"stadt-land-fluss_load","arguments":{"entity":"data","query":{"id":1}}}}'
```
