...
module StadtLandFlussUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = StadtLandFlussResponse.new({})
    ...
    if fetch_err
      ...
      response = StadtLandFlussResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = StadtLandFlussResponse.new(fetched)
    else
