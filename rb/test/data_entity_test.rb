...
class DataEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = StadtLandFlussSDK.test(seed, nil)
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set STADT_LAND_FLUSS_TEST_DATA_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def data_basic_setup(extra)
  ...
  entid_env_raw = ENV["STADT_LAND_FLUSS_TEST_DATA_ENTID"]
  ...
  if env["STADT_LAND_FLUSS_TEST_LIVE"] == "TRUE"
    ...
    client = StadtLandFlussSDK.new(Helpers.to_map(merged_opts))
  end
