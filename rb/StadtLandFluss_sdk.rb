...
require_relative 'StadtLandFluss_types'
...
class StadtLandFlussSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = StadtLandFlussHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = StadtLandFlussHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, StadtLandFlussFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    StadtLandFlussUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = StadtLandFlussHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = StadtLandFlussHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = StadtLandFlussHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => StadtLandFlussError.new(
        "#{op}_allow",
        "StadtLandFlussSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue StadtLandFlussError => err
    ...
    if fetched.is_a?(Hash)
      status = StadtLandFlussHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = StadtLandFlussError.new(
        "graphql_error", "StadtLandFlussSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = StadtLandFlussSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
