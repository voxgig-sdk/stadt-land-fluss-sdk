...
import (
	...
	omni "github.com/voxgig-sdk/stadt-land-fluss-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.StadtLandFlussError); is {
				out := map[string]any{
					"name":    "StadtLandFlussError",
...
func omniCtx(arg any, client *sdk.StadtLandFlussSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.StadtLandFlussSDK); is {
			client = live
