# stadt-land-fluss-cli

boru-driven command-line client **and** interactive REPL for the StadtLandFluss
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/stadt-land-fluss-cli)
make build

# 2. See usage (words, entities, env vars)
./stadt-land-fluss-cli --help

# 3. Provide credentials once, via the environment
export STADT_LAND_FLUSS_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./stadt-land-fluss-cli list data

# 5. Override the API base URL for a single call
STADT_LAND_FLUSS_BASE=https://api.example.com ./stadt-land-fluss-cli list data

# 6. No arguments -> interactive REPL
./stadt-land-fluss-cli
stadt-land-fluss> list data
stadt-land-fluss> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/stadt-land-fluss-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/stadt-land-fluss-cli list data
   ```
...
Run with no arguments to open a REPL (prompt `stadt-land-fluss>`). Each line is
evaluated as its own boru expression:

```text
$ ./stadt-land-fluss-cli
stadt-land-fluss> list data
stadt-land-fluss> /help
stadt-land-fluss> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/stadt-land-fluss-cli`. |
...
(`./stadt-land-fluss-cli <expr>`) and interactively (the REPL), and expressions compose the
