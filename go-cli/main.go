// Command StadtLandFluss-cli is a CLI + REPL for the StadtLandFluss SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      STADT_LAND_FLUSS_APIKEY (api key), STADT_LAND_FLUSS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: STADT_LAND_FLUSS_APIKEY carries the API key and
	// STADT_LAND_FLUSS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("STADT_LAND_FLUSS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("STADT_LAND_FLUSS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewStadtLandFlussSDK(opts)
